...
#     # Koordinatalarni img1_shape dan img0_shape ga qayta masshtablash
#     if ratio_pad is None:  # img0_shape dan hisoblash
#         gain = min(img1_shape[0] / img0_shape[0], img1_shape[1] / img0_shape[1])  # gain = eski / yangi
...
#     coords[:, :4] /= gain
#     coords[:, :4] = coords[:, :4].clamp(0, img0_shape[1])  # koordinatalarni kliplash
#     return coords
...
#         sigma = torch.tensor(KPT_SIGMA, device=true_kpt.device, dtype=true_kpt.dtype)
#         kpt_mask = true_kpt[..., 2] != 0
#         e = (d_x + d_y) / (2 * sigma) ** 2 / (area[:, None, None] + 1e-7) / 2
